...
void message (const char str[1000], int error=-1) {
    if (error == ERROR_MESSAGE){
        cout << "错误：输入错误！你的输入不是数字！";
    }
    else if (error == WARNING_MESSAGE) {
        cout << "警告：你的输入可能不符合规范。（五级制绩点取值一般在0-5）" << endl;
    }
    else if (error == INFO_MESSAGE) {
        cout << "转换成功，百分制绩点是：" << endl;
    }
    else if (error == -1) {
        cout << "请输入五级制绩点：" << endl;
    }
